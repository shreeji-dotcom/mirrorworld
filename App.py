...
def bayesian_crisis_update(state):
    """
    Bayesian-style smoothing update:
    posterior blends prior crisis_prob with an evidence signal.
    """
...
def roll_events(state):
    """
    Behavior-triggered events: max 2 per quarter.
    """
    rng = get_rng(state)
...
def apply_decisions(state, growth, gov_spend, testing, expansion):
    """
    Rule-based + stochastic transitions with compounding risk.
    """
    rng = get_rng(state)

    # Governance: investment raises score; high growth strains controls
    ...
    # Testing: reduces hidden risk + stabilizes trust, but slows growth
    growth_penalty = 0.0
    if testing == "High":
    ...
    # Expansion: wide = faster scaling + more scrutiny
    if expansion == "Wide":
        exp_boost = 1.10
    ...
    # Valuation growth: noisy, trust-dependent
    ...
    # Hidden risk accumulates with fast growth + weak governance
...
def score(state):
    ...
    gov_score = state["governance"]
    penalty = state["reg_pressure"] * 0.30 + state["crisis_prob"] * 0.50 + state["violations"] * 10.0
    total = valuation_score * 0.45 + trust_score * 0.20 + gov_score * 0.20 - penalty * 0.15
...
with left:
    ...
    with c1:
        if st.button("▶️ Run Quarter", use_container_width=True, disabled=state["game_over"]):
            ...
            # Advance quarter if still running
